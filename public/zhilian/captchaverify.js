...
window.captcha = new function () {
    ...
    this.getImage = function () {
        ...
        var randCodeGuid = $.cookie.get('randCodeGuid');
        $.ajax({
            url: common.ajax.appendServerUrl('getRandCode'),
            type: 'GET',
            data: {
                session:session,
                channel:2,
                params:JSON.stringify({'randCodeGuid':randCodeGuid, 'time':collectCaptchaData.startTime+""})
            }
        })
        .done(function(data) {
            _this.bulidSprites('data:image/gif;base64,'+data.data.captcha)
        })
        .fail(function() {
            alter("获取验证码失败")
        })
    ...
    this.submitCode = function (callBack) {
        ...
        var session = $.cookie.get('sc_edaice_session');
        var randCodeGuid = $.cookie.get('randCodeGuid');
        var doType = $.cookie.get('doType');
        var refUrl = $.cookie.get('refUrl');
        var params = JSON.stringify({'randCodeGuid':randCodeGuid, 'p':captcha.p, 'time':captcha.time, 'doType':doType, 'refUrl':refUrl})
        $.ajax({
            url: common.ajax.appendServerUrl('checkRandCode'),
            type: 'GET',
            dataType: 'json',
            data: {
                session:session,
        ...
        .done(function(data) {
            if (data.errorcode == '0'){
                window.top.mytask.$refs.verify.isCommit = false
                window.top.verifySucCallBack()
            }else if(data.errorcode == '15'){
                window.top.mytask.$refs.verify.isCommit = false
                window.top.verifyReloginCallBack()
            }else{
                window.top.mytask.$refs.verify.errText = "验证失败，请重新输入"
				window.top.mytask.$refs.verify.isCommit = false
